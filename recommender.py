...
=======
"""The module that returns the movies"""
from read_and_train import nmf_model, read_and_transform, read__cosim_item_based_model
import pandas as pd
import numpy as np
from scipy.spatial import distance
...
def user_array(movie_ratings, nmf=False):
    ...
    Movie_ratings (dict): name of the movie that the user want to rate as keys (str)
    and raitng of the user for the movie as values - the raings are always 5  (float)
...
sim_mat_item = read__cosim_item_based_model()
...
def get_movies_cosim_item_mix(watched_list_name):
    ...
    similar_movies_id = new_movie.loc['new_movie'].sort_values(ascending=False).head(8).index
    similar_movies_id = [int(x) for x in similar_movies]
...
#     new_user = user_array({'Toy Story (1995)': 1,'Jumanji (1995)': 5 })
#     get_movies_cosim(new_user)
